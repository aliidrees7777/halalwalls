...
// Static fallback used only if the API is unreachable (keeps the UI usable).
const FALLBACK_RES: ResolutionSet = {
  desktop: ["1920×1080", "2560×1440", "3840×2160"],
  mobile: ["1080×2400", "1290×2796", "1320×2868"],
};

// Module-level caches so each list is fetched once per session (shared across
  ...
  useEffect(() => {
    ...
    if (!_catsPromise) {
      _catsPromise = api
        ...
        .catch(() => {
          _catsPromise = null; // allow a later retry
          return [];
        });
    }
    let active = true;
  ...
  useEffect(() => {
    ...
    if (!_resPromise) {
      _resPromise = api
        ...
        .then((d) => {
          const r: ResolutionSet = {
            desktop: d.desktop?.length ? d.desktop : FALLBACK_RES.desktop,
            mobile: d.mobile?.length ? d.mobile : FALLBACK_RES.mobile,
          };
          _resCache = r;
          return r;
        })
