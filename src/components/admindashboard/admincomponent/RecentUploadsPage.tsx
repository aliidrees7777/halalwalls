...
const RecentUploadsPage = () => {
  const columns = [
    ...
    },
    {
      ...
      accessor: (r: any) => (
        <>
          <div className="wtitle">{r.title}</div>
          <div className="wtags">{r.tags}</div>
        </>
      ),
    },
    {
      ...
      accessor: (r: any) => (
        <>
          <div className="uname">{r.user}</div>
          <div className="uemail">{r.email}</div>
        </>
      ),
    },
    {
      header: "Category",
      accessor: (r: any) => (
        <span className={`catb ${r.catClass}`}>{r.category}</span>
      ),
    },
    {
      header: "Resolution",
      accessor: (r: any) => <span className="restext">{r.res}</span>,
    },
    {
      header: "Submitted",
      accessor: (r: any) => (
        <>
          <div className="ddate">{r.date}</div>
          <div className="dtime">{r.time}</div>
        </>
      ),
    },
    {
      header: "Status",
      accessor: (r: any) => <span className="pend-tag">{r.status}</span>,
    },
    {
      header: "Actions",
      accessor: () => (
        <div className="acts">
          <button className="aapprove">
            <svg viewBox="0 0 24 24">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </button>
          <button className="areject">
            <svg viewBox="0 0 24 24">
              <line x1="18" y1="6" x2="6" y2="18" />
              <line x1="6" y1="6" x2="18" y2="18" />
            </svg>
          </button>
          <button className="amore">
